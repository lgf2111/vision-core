...
class CustomerRequestForm(FlaskForm):
    productName = SelectField("Select Product", [validators.DataRequired()], choices=[(
        '123', 'Gigabyte X570 | Aorus Pro Wifi'), ('321', 'Gigabyte X570 | Aorus Pro Wifi')], default='')
    image = FileField('Upload Image', validators=[FileRequired(
    ), FileAllowed(['jpg', 'png', 'jpeg', 'pdf'], 'Images only')])
    issueDesc = TextAreaField('Issue Description', [
                              validators.length(max=200), validators.DataRequired()])
    warranty = SelectField('Warranty Status', [validators.DataRequired()], choices=[
                           ('1', 'Active'), ('0', 'Expired')], default='')
    price = SelectField('Price', [validators.DataRequired()], choices=[
                        ('1', '300.00'), ('0', 'Get A Quote')], default='')

    delivery = SelectField('Doorstep Delivery', [validators.DataRequired()], choices=[
                           ('1', '10.00'), ('0', 'No')], default='')
